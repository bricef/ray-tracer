...
func TestMatrixEquality(t *testing.T) {
	a := math.NewMatrix([][]float64{
		{1, 2, 3},
		{3, 2, 1},
	})
	b := math.NewMatrix([][]float64{
		{1, 2, 3},
		{3, 2, 1},
	})
...
func TestMatrixMult2x2(t *testing.T) {
	...
	expected := math.NewMatrix([][]float64{
		{1*5 + 2*7, 1*6 + 2*8},
		{3*5 + 4*7, 3*6 + 4*8},
	})
	result, err := a.Mult(b)
...
func TestInvertible(t *testing.T) {
	invertible := math.NewMatrix([][]float64{
		{1, 2, 6},
		{-5, 8, -4},
		{2, 6, 4},
	})
	...
	notInvertible := math.NewMatrix([][]float64{
		{2, 6},
		{1, 3},
	})
...
func TestMultInverseYieldsIdentity(t *testing.T) {
	...
	if !result.Equal(math.Identity(4)) {
		t.Errorf("Multiplying %v by its inverse does not yield the identity. Expected %v, got %v", m, math.Identity(4), result)
	}
}
